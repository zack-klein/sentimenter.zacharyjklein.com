...
const steps = ['The backstory', 'Pick a topic', 'Results'];
...
const isLoading = (
  ...
  <Typography color="textSecondary">
    {"Hang tight! Less common topics can take a little while."}
  </Typography>
  <br></br>
  <LinearProgress/>
  </React.Fragment>
...
export default function Main() {
  ...
  async function getSentiments(){
    const encodedText = encodeURIComponent(state.topic);
    try {
      const resp = await fetch(
        `https://api.zacharyjklein.com/sentimenter/tweets/?topic=${encodedText}`
      );
      return resp.json()
    } catch (err) {
      console.log(err);
  ...
  async function handleSubmit() {
    ...
    var sentiments = await getSentiments();
    setState(
      { ...state, activeStep: state.activeStep + 1, isLoading: false, sentiments: sentiments }
    );
  };
